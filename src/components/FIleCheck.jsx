    ...
    aiReport: [
      {
        page: "برگه شرح حال",
        status: "ناقص",
        reason: "مهر و امضای پزشک ندارد",
      },
      { page: "دستورات پزشک", status: "کامل", reason: "-" },
      {
        page: "گزارش پرستاری",
        status: "ناقص",
        reason: "ساعت ثبت دارو درج نشده",
      },
    ],
  },
  {
          ...
          <div key={item.id} className="border-b last:border-b-0">
            <div
              className={`flex items-center p-4 hover:bg-gray-50 transition-colors cursor-pointer ${
                expandedRow === item.id ? "bg-indigo-50" : ""
              }`}
              onClick={() => toggleRow(item.id)}
            >
              <div className="w-1/4 font-mono text-gray-600 font-bold">
                {item.caseNumber}
              </div>
              ...
              <div className="bg-gray-50 p-6 border-t animate-fade-in-down cursor-default">
                ...
                <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
                  ...
                  {item.aiReport.map((report, idx) => (
                    <div
                      key={idx}
                      className="flex items-center py-3 px-4 border-b last:border-none hover:bg-gray-50 text-sm"
                    >
